...
module.exports = {
  ...
  activate(state) {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      algorithm: "command-t",
      confirmAction: "fuzzy-files:open",
      additionalActionCommands: ["fuzzy-files:clear-recent"],
      ...
      actionsFilter: ({ name }) => {
        if (name === "fuzzy-files:clear-recent") return this.recentlyUsed.length > 0;
        return (
          name !== "fuzzy-files:remove-from-recent" ||
    ...
    this.disposables = new CompositeDisposable(
      ...
      lumine.commands.add(this.selectList.element, {
        ...
        "fuzzy-files:remove-from-recent": {
          description: "Drop the selected file from the recent section, leaving the list open.",
  ...
  isRecent(item) {
    return item != null && this.recentlyUsed.includes(item.aPath);
  },
  ...
  // Forgetting one file is something a user does to several in a row, so the
  ...
  removeFromRecent() {
    ...
    this.recentlyUsed.splice(this.recentlyUsed.indexOf(aPath), 1);
    this.viewSynced = false;
    if (this.selectList.isVisible()) this.syncList();
    // The sync rebuilds the rows, so the row to keep selected is found again
    // by path rather than by the object the action started from.
    const row = this.selectList.items?.find((each) => each.aPath === aPath);
  ...
  performAction(mode, params) {
    ...
    // each branch, where a new action would forget to. A trashed file counts
    // too — the trash is where it is put back from.
    ...
    if (mode === "open") {
      ...
      if (params.effect === "cut") {
        this.nativeClipService.cutPaths([aPath]);
      } else if (params.effect === "copy") {
        this.nativeClipService.copyPaths([aPath]);
      }
