...
module.exports = {
  ...
  activate(state) {
    ...
    this.disposables = new CompositeDisposable(
      ...
      lumine.config.onDidChange("core.ignoredNames", () => {
        this.restartCache();
      }),
      lumine.config.onDidChange("core.followSymlinks", () => {
        this.restartCache();
      }),
      lumine.config.onDidChange("core.excludeVcsIgnoredPaths", () => {
        this.restartCache();
      }),
  ...
  cache(callback) {
    ...
    if (lumine.project.getPaths().length === 0) {
      this.building = false;
      this.viewSynced = false;
    ...
    try {
      ...
      if (error.code === "ENOENT" || error.code === "EPERM") {
        lumine.notifications.addError("Project path not found!", { detail: error.message });
      } else {
        throw error;
      }
  ...
  startLoadPathsTask() {
    ...
    if (lumine.project.getPaths().length === 0) return;
    this.cache();
  },
  ...
  updateEvent(events) {
    ...
    for (let e of events) {
      if (e.action === "created") {
        [pPath, fPath] = lumine.project.relativizePath(e.path);
        ...
        [pPath, fPath] = lumine.project.relativizePath(e.path);
        if (!pPath) continue;
        lumine.icons.invalidate({ paths: [e.path] });
        ...
        let [pOldPath, fOldPath] = lumine.project.relativizePath(e.oldPath);
        let [pNewPath, fNewPath] = lumine.project.relativizePath(e.path);
        lumine.icons.invalidate({ paths: [e.oldPath] });
